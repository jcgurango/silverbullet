...
export function showMergeView(
  ...
  function saveResolved() {
    ...
    // No need to set the parent hash here: the sync engine already wrote
    // `syncedHashes[path] = serverHash` into the snapshot when it received the
    // 409, and the hash-tracking hook on HttpSpacePrimitives will attach it as
    // X-Parent-Hash on the next write.
    void serverHash;
